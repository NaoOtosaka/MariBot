...
<p align="center">
    <a href="https://github.com/CMHopeSunshine/nonebot-plugin-learning-chat/">
        <img src="https://github.com/A-kirami/nonebot-plugin-template/raw/resources/nbp_logo.png"
...
body = Wrapper(className="w-2/5 mx-auto my-0 m:w-full", body=login_form.to_dict())
login_page = Page(title="", body=[logo.to_dict(), body.to_dict()])
...
global_config_form = Form(
    ...
    actions=[
        Action(label="保存", level="success", actionType="confirm"),
        Action(label="重置", level="warning", actionType="cancel"),
    ],
)
group_select = Select(
...
blacklist_table = CRUD(
    ...
    columns=[
        ...
        Table.Column(type="text", label="已禁用的群", name="bans", searchable=True),
    ],
)
message_table = CRUD(
    mode="table",
    ...
    footable=True,
    columns=[
        Table.Column(type="text", label="消息ID", name="message_id"),
        Table.Column(type="text", label="群ID", name="group_id", searchable=True),
        Table.Column(type="text", label="用户ID", name="user_id", searchable=True),
        Table.Column(
            type="tpl",
...
answer_table = CRUD(
    ...
    columns=[
        Table.Column(type="text", label="ID", name="id", visible=False),
        Table.Column(type="text", label="群ID", name="group_id", searchable=True),
        Table.Column(
            type="tpl",
            tpl="${keywords|truncate:20}",
        ...
        Table.Column(type="text", label="次数", name="count", sortable=True),
        AmisList(
            label="完整消息",
            name="messages",
            breakpoint="*",
            source="${messages}",
            listItem=AmisList.Item(type='text', body=[AmisList.ListBodyField(type='text', name="msg")]),
        ),
    ],
)
answer_table_on_context = CRUD(
    mode="table",
    syncLocation=False,
    footable=True,
    ...
    columns=[
        Table.Column(type="text", label="ID", name="id", visible=False),
        Table.Column(type="text", label="群ID", name="group_id"),
        Table.Column(
            type="tpl",
            tpl="${keywords|truncate:20}",
            label="内容/关键词",
...
context_table = CRUD(
    ...
    columns=[
        ...
        Table.Column(type="text", label="已学次数", name="count", sortable=True),
    ],
)
...
config_page = PageSchema(
    ...
    schema=Page(
        ...
        interval=60000,
        body=[
            global_config_form.to_dict(),
            group_select.to_dict(),
            group_config_form.to_dict()
        ],
    ),
)
...
admin_app = App(
    ...
    header=header,
    pages=[{"children": [config_page.to_dict(), database_page.to_dict()]}],
