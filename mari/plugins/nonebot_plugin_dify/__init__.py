...
async def ignore_rule(event: Event) -> bool:
    ...
    # at 始终触发
    if event.is_tome() and config.dify_use_at_rule:
        return True

    # 消息以指定词开头
    if msg.startswith(config.dify_rule_prefix):
        return True
...
async def _(
    ...
    msg_plaintext: str = event.message.extract_plain_text()
    if not config.dify_use_at_rule:
        msg_plaintext = msg_plaintext.removeprefix(config.dify_rule_prefix)
    if msg_plaintext == "":
