...
wb_set_cmd_gl = on_regex(
    r"^((?:全局|模糊|正则|@)*)\s*我问\s*(\S+.*?)\s*你答\s*(\S+.*?)\s*$",
    flags=re.S,
    block=True,
    priority=10,
...
wb_search_cmd = on_regex(
    r"^看看\s*((?:群|用户)*)\s*(\d*)\s*((?:全局)?(?:模糊|正则)?@?)\s*我问\s*(.*?)\s*$",
    flags=re.S,
    block=True,
    priority=10,
    permission=PERM_GLOBAL,
...
async def wb_search(
    ...
    if isinstance(event, GroupMessageEvent):
        ...
        for entry in entrys:
            forward_msg.append(
                to_json(
                    "问: " + entry.key, nickname or "user", str(event.user_id)
                )
            )
