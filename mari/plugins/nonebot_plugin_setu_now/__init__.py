...
from typing import Any, Union, Annotated, Tuple
from pathlib import Path

from PIL import UnidentifiedImageError
from nonebot import on_regex, on_command
from nonebot.log import logger
from nonebot.params import Depends, RegexGroup, Command
...
contain_ai_matcher = on_command(
    "不要AI", aliases={"可以有AI"}, permission=SUPERUSER
)


@contain_ai_matcher.handle()
async def _(cmd: Tuple[str, str] = Command()):
    _, action = cmd
    if action == '不要AI':
        config.setu_contain_ai = False
        await contain_ai_matcher.finish("AI被鲨掉啦")
    elif action == "可以有AI":
        config.setu_contain_ai = True
        await contain_ai_matcher.finish("现在可以看到AI图片了")
